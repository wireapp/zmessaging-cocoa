...
@objc open class ClientUpdateStatus: NSObject {
        ...
        if isFetchingClients {
            ...
            if needsToVerifySelfClient {
                do {
                    ...
                    ZMClientUpdateNotification.notifyFetchingClientsCompleted(userClients: excludingSelfClient, context: syncManagedObjectContext.zm_userInterface)
                }
                catch let error as NSError {
                    ZMClientUpdateNotification.notifyFetchingClientsDidFail(error: error, context: syncManagedObjectContext.zm_userInterface)
                }
            }
            else {
                ZMClientUpdateNotification.notifyFetchingClientsCompleted(userClients: clients, context: syncManagedObjectContext.zm_userInterface)
            }
        }
    }
    ...
    public func deleteClients(withCredentials emailCredentials:ZMEmailCredentials) {
        if (emailCredentials.password?.characters.count)! > 0 {
            ...
            ZMClientUpdateNotification.notifyDeletionFailed(error: ClientUpdateError.invalidCredentials.errorForType(), context: syncManagedObjectContext.zm_userInterface)
        }
    }
        ...
        if let errorCode = ClientUpdateError(rawValue: error.code) , error.domain == ClientUpdateErrorDomain {
            if  errorCode == .clientToDeleteNotFound {
                ...
                if !hasClientsToDelete {
                    ...
                    ZMClientUpdateNotification.notifyDeletionCompleted(remainingClients: selfUserClientsExcludingSelfClient, context: syncManagedObjectContext.zm_userInterface)
                }
            }
            else if  errorCode == .invalidCredentials {
                isWaitingToDeleteClients = false
                internalCredentials = nil
                ZMClientUpdateNotification.notifyDeletionFailed(error: error, context: syncManagedObjectContext.zm_userInterface)
            }
        }
    }
