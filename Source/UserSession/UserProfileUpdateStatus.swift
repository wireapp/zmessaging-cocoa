...
@objc public class UserProfileUpdateStatus : NSObject {
    ...
    /// should be removing email
    fileprivate(set) var removeEmail : Bool?
...
extension UserProfileUpdateStatus : UserProfile {
    ...
    public func requestEmailRemoval() {
        self.managedObjectContext.performGroupedBlock {
            let selfUser = ZMUser.selfUser(in: self.managedObjectContext)
            guard selfUser.phoneNumber != nil else {
                self.didFailEmailUpdate(error: UserProfileUpdateError.removingLastIdentity)
                return
            }
            
            self.removeEmail = true
            self.newRequestCallback()
        }
    }
...
extension UserProfileUpdateStatus {
    ...
    /// Invoked when the request to change email was sent successfully
    func didRemoveEmailSuccessfully() {
        self.removeEmail = nil
        UserProfileUpdateNotification.post(type: .didRemoveEmail)
        self.managedObjectContext.selfContactCardNeedsToBeUploaded = true
    }

    /// Invoked when the request to change email failed
    func didFailEmailRemoval(error: Error) {
        self.removeEmail = nil
...
extension UserProfileUpdateStatus {
    ...
    /// Whether we are currently removing email
    public var currentlyRemovingEmail : Bool {
        guard self.selfUserHasPhoneNumber else {
            return false
        }
        return self.removeEmail == true
    }
