...
@objcMembers public class UserProfileUpdateStatus : NSObject {
    ...
    /// The legal hold consent value to set.
    private(set) var legalHoldExposureConsentToSet: Bool?
...
extension UserProfileUpdateStatus : UserProfile {
    ...
    public func requestLegalHoldExposureConsentChange(isConsentGiven: Bool) {
        self.managedObjectContext.performGroupedBlock {
            self.legalHoldExposureConsentToSet = isConsentGiven
            self.newRequestCallback()
        }
    }
...
extension UserProfileUpdateStatus {
    ...
    /// Invoked when the legal hold exposure consent successfully set.
    func didSetLegalHoldExposureConsent() {
        guard let isConsentGiven = legalHoldExposureConsentToSet else { return }
        ZMUser.selfUser(in: self.managedObjectContext).legalHoldConsent = isConsentGiven ? .given : .notGiven
        self.legalHoldExposureConsentToSet = nil
    }

    /// Invoked when the legal hold exposure consent was not set because of a generic error.
    func didFailToSetLegalHoldExposureConsent() {
        self.legalHoldExposureConsentToSet = nil
    }
...
extension UserProfileUpdateStatus {
    ...
    /// Whether we are currently requesting a change of the legal hold exposure consent.
    public var currentlySettingLegalHoldExposureConsent: Bool {
        return self.legalHoldExposureConsentToSet != nil
    }
