        ...
        case let .joinConversation(key: key, code: code):
            ZMConversation.fetchIdAndName(key: key,
                ...
                case .success((let conversationId, let conversationName)):
                    /// First of all, we should try to fetch the conversation with ID from the response. If the conversation doesn't exist, we should initiate a request to join the conversation
