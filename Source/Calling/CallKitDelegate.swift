...
public class CallKitDelegate : NSObject {
    ...
    fileprivate unowned let sessionManager : SessionManager
    ...
    public convenience init(sessionManager: SessionManager, flowManager: FlowManagerType?, mediaManager: AVSMediaManager?) {
...
extension CallKitDelegate {
    ...
    func conversationAssociated(with contacts: [INPerson]) -> ZMConversation? {
        ...
        guard contacts.count == 1,
              ...
              let remoteIdentifier = UUID.init(uuidString: customIdentifier),
              let managedObjectContext = sessionManager.activeUserSession?.managedObjectContext
        else {
            return nil
        }
        
        return ZMConversation(remoteID: remoteIdentifier, createIfNeeded: false, in: managedObjectContext)
    }
    ...
    public func callCenterDidChange(callState: CallState, conversation: ZMConversation, user: ZMUser?, timeStamp: Date?) {
        ...
        case .incoming(video: let video, shouldRing: let shouldRing, degraded: _):
            ...
            if shouldRing {
                ...
                if let callUUID = callUUID(for: conversation) { // FIXME jacob, clean up
                    provider.reportCall(with: callUUID, endedAt: timeStamp, reason: .unanswered)
                }
            }
        case let .terminating(reason: reason) where !(reason == .normal && user!.isSelfUser): // FIXME jacob, dont force unwrap
            if let callUUID = callUUID(for: conversation) { // FIXME clean up
                provider.reportCall(with: callUUID, endedAt: timeStamp, reason: reason.CXCallEndedReason)
            }
        default:
            break
        }
