...
public protocol AVSWrapperType {
    ...
    func close()
    func received(callEvent: CallEvent) -> CallReceivedResult
...
public class AVSWrapper: AVSWrapperType {
    ...
    public func received(callEvent: CallEvent) -> CallReceivedResult {
        var result: CallReceivedResult = .unknown
        callEvent.data.withUnsafeBytes { (bytes: UnsafePointer<UInt8>) in
            ...
            result = CallReceivedResult(wcall_error: wcall_recv_msg(handle, bytes, callEvent.data.count, currentTime, serverTime, callEvent.conversationId.transportString(), callEvent.userId.transportString(), callEvent.clientId))
            /// TODO Next line just for testing purposes. Remove before merging.
            result = CallReceivedResult.unknownProtocol
        }
        return result
    }
