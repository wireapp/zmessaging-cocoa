...
class TeamDownloadRequestStrategyTests: MessagingTest {
    ...
    func testThatItRemovesAMemberThatIsSelfUser() {
        
        let teamId = UUID.create()
        let userId = UUID.create()
        let conversationId = UUID.create()
        let expectation = self.expectation(description: "notifcation received")
        var authToken: ZMAuthenticationObserverToken!
        
        syncMOC.performGroupedBlockAndWait {
            // given
            authToken = ZMUserSessionAuthenticationNotification.addObserver { notification in
                expectation.fulfill()
                XCTAssertEqual(notification.error! as NSError, NSError.userSessionErrorWith(.clientDeletedRemotely, userInfo: .none))
            }
            ...
            // then
            let fetchedTeam = Team.fetchOrCreate(with: teamId, create: false, in: self.syncMOC, created: nil)
            XCTAssertNil(fetchedTeam)
            
            let fetchedConversation = ZMConversation.fetch(withRemoteIdentifier: conversationId, in: self.syncMOC)
            XCTAssertNil(fetchedConversation)
            
            XCTAssertEqual(self.mockApplicationStatus.deletionCalls, 1)
        }
