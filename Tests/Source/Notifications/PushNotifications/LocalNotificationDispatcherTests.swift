...
extension LocalNotificationDispatcherTests {
    ...
    func testThatItCancelsNotificationWhenUserDeletesLike() {
        ...
        let reaction1 = GenericMessage(content: WireProtos.Reaction.createReaction(emoji: "❤️", messageID: message.nonce!) as! MessageCapable)
        let reaction2 = GenericMessage(content: WireProtos.Reaction.createReaction(emoji: "", messageID: message.nonce!) as! MessageCapable)
